# -*- coding: utf-8 -*-
"""
Created on Wed Feb 11 22:09:42 2026
...
# -*- coding: utf-8 -*-
"""
Balanced Lightweight Fog ML Models on weak-jamming dataset

Directory: D:\test\merged_features\scaled\cleaned
Files: merged_features_part*_scaled_cleaned.csv

Features (example): extended IQ features
Labels: Condition (e.g. AWGN / Deceptive / NoJam)

Per-file StratifiedKFold CV, balanced RF / NB / LR, detailed metrics,
aggregated confusion matrices, and Fog vs Cloud comparison plots.
"""
...
results_dir = os.path.join(data_dir, "fog_results_cv")
os.makedirs(results_dir, exist_ok=True)

# Use the features you now generate (you can remove some if needed)
features_to_use = [
    "Amplitude",
    "Phase",
    "Power",
...
label_column = "Condition"  # change if needed
...
# Cloud numbers just for plotting comparison (replace with your real cloud summary if you have it)
...
# Prepare label encoder – put your actual class names here
...
for file_path in files:
    ...
    for model_name, model in models.items():
        ...
        # Save per-file confusion matrix
        fig, ax = plt.subplots(figsize=(5, 4))
        sns.heatmap(
            agg_cm_norm, annot=True, fmt=".3f", cmap="Blues",
            xticklabels=classes_labels, yticklabels=classes_labels, ax=ax
        )
        ...
        if len(probs_list) > 0:
            ...
            roc_file = os.path.join(results_dir, f"roc_data_{model_name.replace(' ', '_')}_{file_name}.npz")
            np.savez(roc_file, y=Ys, proba=Probas)

# ===================== SAVE SUMMARY AND PLOTS ==============================
results_df = pd.DataFrame(all_results)
results_csv = os.path.join(results_dir, "fog_model_cv_summary_full.csv")
results_df.to_csv(results_csv, index=False)
print(f"\nSaved CSV results to: {results_csv}")

metrics_cols = ["accuracy_mean", "precision_mean", "recall_mean", "f1_mean", "auc_mean"]
summary_per_model = results_df.groupby("model")[metrics_cols].mean().reset_index()
...
combined_plot_df = pd.concat([fog_rows, cloud_rows], ignore_index=True)
...
for j, metric in enumerate(metrics):
    ax.bar(indices + (j - n_metrics/2)*width + width/2, vals[:, j], width, label=metric)
ax.set_xticks(indices)
ax.set_xticklabels(labels, rotation=45, ha='right')
ax.set_ylabel("Metric value")
ax.set_ylim(0, 1.05)
ax.set_title("Combined metrics: Fog (light) vs Cloud (heavy) models")
ax.legend(loc='upper left', bbox_to_anchor=(1.01, 1), frameon=False)
plt.tight_layout()
combined_path = os.path.join(results_dir, "fig_combined_metrics_fog_cloud.png")
