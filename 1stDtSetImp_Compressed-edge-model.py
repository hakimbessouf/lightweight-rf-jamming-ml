...
- Trains models (RF, Naive Bayes, Logistic Regression) per fold, computes per-fold metrics
...
- Saves results CSVs and generates figures:
    - Combined bar chart (all metrics) for Edge vs Cloud (cloud numbers can be provided / realistic Table4 used)
    - Separate bar charts per metric
    - Confusion matrices (edge RF aggregated and cloud RF aggregated if cloud file available)
...
features_to_use = ["Amplitude", "Phase", "Power", "Instantaneous_Frequency", "Distance_Tx_Rx"]
...
# Models to evaluate (Edge lightweight)
models = {
